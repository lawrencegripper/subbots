...
//        "channelId": activityReq.channelId,
        "channelId": "webchat",
        "timestamp": activityReq.body.localTimestamp,
        "from": { //What do I do here?
        ...
        "recipient": {
          "id": "lgtestbot2@TCBIhtTmcW4",
          "name": "Testbot" //?? What to do here
        },
        "text": "Routed to " + subbotId
      };
    subBotRequest.body.conversation.id = conv.convId;
    // subBotRequest.body.serviceUrl = `${this._serviceURL}/${activityReq.body.channelId}`;
    // subBotRequest.body.channelId = "emulator";
  ...
  _replyToActivity(req, res, next) {
    ...
    if (conv) {
      ...
      if (this._isSubBotSwitchReply(req)) {
      ...
      if (req.body.name === "debug" && req.body.type === "event") {
        console.info("Debug event ignoring")
      } else {
